...
int main(int argc, char *argv[])
{
	...
	char buffer[MAXBUFFLEN];//largest plaintext file is 70,000
	char buffer2[MAXBUFFLEN];//contains key
	char indicator[5];//contains e that indicates encryption and @ separator
	...
	//store plaintext and keygen in final buffer to be sent
	strcat(buffer,buffer2);
	strcat(buffer,"$");//add deliminator

	//send indicator of encrption, "handshake"and echo back
	send(socketFD,indicator,strlen(indicator),0);
	recv(socketFD,indicator,sizeof(indicator),0);
	//check if otp_enc is authorized to communicate w/ port/"server"
	...
	// Send message to otp_enc_d
	int accWritten = 0; //number of characters actually written
	...
	int totChar = strlen(buffer);//# characters to receive
	// Get return message from server
	memset(buffer, '\0', sizeof(buffer)); // Clear out the buffer again for reuse
	char readBuffer[1000];
	int actRead = 0;//#characters actually read
	//receive all characters in chunks
